import { IsString, IsEmail, IsOptional, IsNumber, IsIn, IsNotEmpty, IsDateString } from 'class-validator';
...
export class CreateEmployeeDto {
  ...
  @ApiPropertyOptional({ description: 'Photo path', example: '/photos/john.jpg' })
  @IsOptional()
  @IsString()
  photoPath?: string;

  @ApiProperty({ description: 'Employee type', enum: ['Employee', 'Partner'], example: 'Employee' })
  @IsIn(['Employee', 'Partner'], { message: 'Type must be Employee or Partner' })
  type: 'Employee' | 'Partner';

  @ApiPropertyOptional({ description: 'Employee status', enum: ['Active', 'Inactive'], example: 'Active' })
  @IsOptional()
  @IsIn(['Active', 'Inactive'], { message: 'Status must be Active or Inactive' })
  status?: 'Active' | 'Inactive';

  @ApiProperty({ description: 'Work email address', example: 'john.doe@company.com' })
  @IsEmail({}, { message: 'Invalid email format' })
  @IsNotEmpty({ message: 'Work email is required' })
  workEmail: string;

  @ApiProperty({ description: 'Hire date in ISO format', example: '2024-01-15' })
